...
class TimeEncode(nn.Module):
    def __init__(self):
        ...
        def forward(self, points):
            feat1 = self.MLP(points) # B*num_poitns*1*1024
            g_feat = torch.max(feat1, 2, keepdim=True)[0]#self.MAXPOOL1(feat1) # B*1024
            g_feat = g_feat.view(-1, 1024)
            # att_feat = self.att(g_feat)
            g_feat = self.FC(g_feat)
            return att_feat #B*num_point*12
...
class TiisEncoder(nn.Module):
    r"""The Encoder for TiisNet 
    `"
    .. note::
    Args:
        in_channels (int, optional): input channels . Defaults to .
    ...
    def __init__(self,
        ...
        for i in range(len(blocks)):
            ...
            # ti_encoder.append(self._make_pe_enc(  # time_line encoder
            #     block, channels[i], blocks[i], stride=strides[i], group_args=group_args,
            #     is_head=i == 0 and strides[i] == 1
            # ))
        ...
        # FOR TIME ENCODER
        # ti_encoder = [] # time_line encoder
        # ti_encoder.append(TimeEncode())

        self.encoder = nn.Sequential(*encoder)
        self.pe_encoder = pe_encoder #nn.Sequential(*pe_encoder)
        # self.ti_encoder =  nn.Sequential(*ti_encoder)
        self.pe_grouper = pe_grouper
        self.out_channels = channels[-1]
        self.channel_list = channels
        #################
        self.conv0_1 = torch.nn.Conv1d(3, 64, 1)
    ...
    def _time_encode(self,points):
        
        B, D, N = points.shape
        x = F.relu(self.bn0_1(self.conv0_1(points)))
    ...
    def forward_seg_feat(self, p0, f0=None):
        if hasattr(p0, 'keys'):
            p0, f0, t0 = p0['pos'], p0.get('x', None), p0.get('time', None)
            # print(t0.shape)  #[100, 1, 2048]
            # t0 =p0.['time'] # N P T
        if f0 is None:
            f0 = p0.clone().transpose(1, 2).contiguous()
        if t0 is None:
            print('error! No time_line found')
        p, f = [p0], [f0]
        for i in range(0, len(self.encoder)):
            if i == 0:
                pe = None
                _p, _f, _ = self.encoder[i]([p[-1], f[-1], pe])
                # print(_p.shape) #([100, 2048, 3])
                
                ti_feat =self._time_encode(t0)
                # print(ti_feat.shape) # [100,12,2048]
                # print(_f.shape) #([100, 32, 2048])
                
                _f = _f + ti_feat
                # _f = torch.cat([_f, ti_feat], 1)       # time
            else:
                ...
                if self.blocks[i] > 1:
                    ...
                    # conv on time_scale clicks
                    # ft = self.ti_encoder[i](t0) #time
                    # _f = torch.cat([_f, _ft], 1) #time
...
class TiisEncoder2(nn.Module):
    ...
    def _time_encode(self,points):
        ...
        x = x.view(-1, 1024)
        x = F.relu(self.bn4(self.fc1(x)))
        x = F.relu(self.bn5(self.fc2(x)))  # OUT [100,256]
        # att
        x = x.reshape(B,-1,1) # [100,256,1]
        att = x.transpose(1,2) # [100,1,256]
        att =  torch.bmm(x, att) # 100,256,256
        x = torch.bmm(att,x)   # [100,256,1]
        x = x.reshape(B,-1) # [100,256]
        # ----
        x = self.fc3(x)  # [100,16]
        x = x.reshape(B,-1,1)
